...
primitive_roots = [z for z in {v for v in range(1, q) if gcd(v, q) == 1} if{pow(z, w, q) for w in range(1, q)} == {v for v in range(1, q) if gcd(v, q) == 1}]
...
for sender, public_key in enumerate(y):
    for recipient in range(0, users):
        if recipient != sender and recipient > sender:
            print("User {0} (X = {1}, Y = {2}) is communicating with User {3} (X = {4}, Y = {5})".format(sender+1,x[sender],y[sender],recipient+1,x[recipient],y[recipient]))
